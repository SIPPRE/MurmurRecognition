...
class ClassificationApp:
    ...
    def load_data(self):
        ...
        # Φιλτράρισμα των δεδομένων για να περιλαμβάνονται μόνο όσα έχουν φύσημα και κατηγορία NormalOrAbnormal
        self.df_features = self.df_features[self.df_features['NormalOrAbnormal'].notna()]
    ...
    def plot_full_stats(self):
        ...
        ax.bar(r1, accuracies, color='b', width=bar_width, edgecolor='grey', label='Accuracy')
        ax.bar(r2, f1_scores, color='g', width=bar_width, edgecolor='grey', label='F1 Score')
        ax.bar(r3, precisions, color='r', width=bar_width, edgecolor='grey', label='Precision')
        ax.bar(r4, recalls, color='c', width=bar_width, edgecolor='grey', label='Recall')
        ax.bar(r5, specificities, color='m', width=bar_width, edgecolor='grey', label='Specificity')
        ...
        for i in range(len(accuracies)):
            ...
            ax.text(i + bar_width, f1_scores[i] + 0.01, f'{f1_scores[i]:.2f}', ha='center', va='bottom')
            ax.text(i + 2 * bar_width, precisions[i] + 0.01, f'{precisions[i]:.2f}', ha='center', va='bottom')
            ax.text(i + 3 * bar_width, recalls[i] + 0.01, f'{recalls[i]:.2f}', ha='center', va='bottom')
            ax.text(i + 4 * bar_width, specificities[i] + 0.01, f'{specificities[i]:.2f}', ha='center', va='bottom')
